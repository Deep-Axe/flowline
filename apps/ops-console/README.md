...
Angular 22 control-room UI. Venue, model status, replay, reset, camera-frame analysis, and incident alerts go through **Apollo / GraphQL**. Replay uses the `demoPlayback` WebSocket subscription; `analyzeCamera` uses the GraphQL multipart spec. Node **24.19.0** or newer (`^22.22.3 || ^24.15.0`).
